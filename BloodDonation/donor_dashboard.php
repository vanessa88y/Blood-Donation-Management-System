...
$sql_donations = "SELECT * FROM donations WHERE id = '$id'";
$result_donations = $con->query($sql_donations);

// Query to retrieve pending requests
$sql_pending = "SELECT * FROM donations WHERE id = '$id' AND status = 'pending'";
$result_pending = $con->query($sql_pending);

// Query to retrieve accepted requests
$sql_accepted = "SELECT * FROM donations WHERE id = '$id' AND status = 'accepted'";
$result_accepted = $con->query($sql_accepted);

// Query to retrieve rejected 
$sql_rejected = "SELECT * FROM donations WHERE id = '$id' AND status = 'rejected'";
        ...
        <div class="box">
            ...
            <ul>
                ...
                if ($result_donations->num_rows > 0) {
                    ...
                    echo "<li>No blood donations yet</li>";
                }
                ?>
            </ul>
        </div>
